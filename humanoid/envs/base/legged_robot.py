...
class LeggedRobot(BaseTask):
    ...
    def reset_idx(self, env_ids):
        ...
        if self.cfg.domain_rand.randomize_gains:
            self.Kp_factors[env_ids] = torch_rand_float(self.cfg.domain_rand.p_gain_range[0], self.cfg.domain_rand.p_gain_range[1], (len(env_ids), 1), device=self.device)
            self.Kd_factors[env_ids] = torch_rand_float(self.cfg.domain_rand.d_gain_range[0], self.cfg.domain_rand.d_gain_range[1], (len(env_ids), 1), device=self.device)
        if self.cfg.domain_rand.randomize_motor_strength:
            self.motor_strength[env_ids] = torch_rand_float(self.cfg.domain_rand.motor_strength_range[0], self.cfg.domain_rand.motor_strength_range[1], (len(env_ids), 1), device=self.device)
    ...
    def _post_physics_step_callback(self):
        ...
        self.zero_cmd_ids = torch.nonzero(torch.norm(self.commands[:, :3], dim=1) <= self.min_cmd_vel).squeeze(1)
    ...
    def _init_buffers(self):
        ...
        self.feet_contact_time = torch.zeros(self.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _reward_stand_still(self):
        ...
        rew[self.zero_cmd_ids] = torch.sum(torch.abs(self.dof_pos[self.zero_cmd_ids] - self.default_dof_pos), dim=1)
        return rew
